...
// === Mobile carousel helper (final): ensure first items visible + add progress bar, even on late DOM ===
(function(){
  ...
  function enhanceAll(){
    try{
      ...
      if (!isMobile) return;
      document.querySelectorAll('.posicionamento-options').forEach(enhanceContainer);
    }catch(e){}
  }
  function enhanceContainer(container){
    if (!container || container._enhanced) return;
    container._enhanced = true;

    // Clean empties (defensive)
    Array.from(container.children).forEach(function(node){
      if (node.classList && node.classList.contains('overcell')){
        if (!node.querySelector('input[type="radio"]')) node.remove();
      }
    });
    ...
    if (!container._track){
      ...
      container.parentElement && container.parentElement.insertBefore(track, container.nextSibling);
      container._track = track;
      function update(){
        var max = container.scrollWidth - container.clientWidth;
        var ratio = max>0 ? (container.scrollLeft/max) : 0;
        var visible = Math.min(1, container.clientWidth / container.scrollWidth);
        thumb.style.width = (visible*100) + '%';
        thumb.style.transform = 'translateX(' + (ratio * (100 - visible*100)) + '%)';
      }
  ...
  // Run on DOM ready, and again after small delay (in case of late content)
  ready(function(){
    enhanceAll();
    setTimeout(enhanceAll, 200);
    setTimeout(enhanceAll, 600);
  });
  // Observe late insertions dynamically
  try {
      ...
      muts.forEach(function(m){
        if (m.addedNodes && m.addedNodes.length){
          m.addedNodes.forEach(function(n){
            if (n.nodeType===1){
              if (n.matches && n.matches('.posicionamento-options')) enhanceContainer(n);
              n.querySelectorAll && n.querySelectorAll('.posicionamento-options').forEach(enhanceContainer);
            }
          });
        }
      });
