...
/* ===== Mobile collapse for .posicionamento-options (dynamic, via MutationObserver) ===== */
(function(){
  function isMobile(){ return window.matchMedia('(max-width: 768px)').matches; }
  ...
  function setupContainer(container){
    ...
    // Ensure button is after the grid inside the same overcell
    ...
    function apply(){
      ...
      if (container.classList.contains('expanded')){
        ...
        // Fallback min height in case children not rendered yet
  ...
  // Observe dynamic inserts (build renders after load)
