...
// === Rebuilt mobile carousel for posicionamento-options ===
(function(){
  function wrapAndInit(container){
    if (container.closest('.pos-op-viewport')) return; // already wrapped
    var viewport = document.createElement('div');
    viewport.className = 'pos-op-viewport';
    var wrapper = document.createElement('div');
    wrapper.className = 'pos-op-carousel';
    ...
    track.className = 'posicionamento-progress';
    var bar = document.createElement('div');
    bar.className = 'posicionamento-progress__bar';
    track.appendChild(bar);
    wrapper.appendChild(track);

    // make every child a slide
    var slides = Array.prototype.slice.call(container.children);
    slides.forEach(function(el){ el.classList.add('pos-op-slide'); });

    // state
    var index = 0;
    var translate = 0;
    var step = 0;  // px including gap
    var maxIndex = 0;
    var dragging = false;
    var startX = 0;
    var startT = 0;
    ...
    function compute(){
      ...
      if (!first) { step = 0; maxIndex = 0; updateBar(); return; }
      var r1 = first.getBoundingClientRect();
      if (second){
        var r2 = second.getBoundingClientRect();
        step = (r2.left - r1.left);
        if (step <= 0) step = r1.width + 12;
      } else {
        step = r1.width + 12;
      }
      var visible = viewport.clientWidth / step;
      maxIndex = Math.max(0, Math.ceil(container.children.length - visible));
      index = Math.min(index, maxIndex);
      translate = -index * step;
      container.style.transition = 'transform .25s ease';
      container.style.transform = 'translateX(' + translate.toFixed(2) + 'px)';
      updateBar();
    }
    ...
    function updateBar(){
      ...
      if (!total) return;
      var ratioVisible = Math.min(1, viewport.clientWidth / (total * step || 1));
      bar.style.width = (Math.max(0.1, ratioVisible) * 100).toFixed(2) + '%';
      var travel = 100 - parseFloat(bar.style.width);
      var pos = (maxIndex === 0) ? 0 : (index / maxIndex) * travel;
      bar.style.left = pos.toFixed(2) + '%';
    }

    function snap(){
      index = Math.max(0, Math.min(maxIndex, Math.round(-translate / step)));
      translate = -index * step;
      container.style.transition = 'transform .25s ease';
      container.style.transform = 'translateX(' + translate.toFixed(2) + 'px)';
      updateBar();
    ...
    function onMove(e){
      ...
      if (Math.abs(dx) > 5) clickCanceled = true;
      e.preventDefault && e.preventDefault(); // stop page panning
      translate = startT + dx;
      var maxLeft = 0;
      var maxRight = -maxIndex * step;
      if (translate > maxLeft) translate = maxLeft + (translate - maxLeft) * 0.2;        // rubber
      if (translate < maxRight) translate = maxRight + (translate - maxRight) * 0.2;    // rubber
      container.style.transform = 'translateX(' + translate.toFixed(2) + 'px)';
      updateBar();
    }
    function onUp(e){
  ...
  function init(){
    ...
    document.querySelectorAll('.posicionamento-options').forEach(wrapAndInit);
  }
