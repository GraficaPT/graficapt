...
(function(){
  ...
  function mount(){
    const pingAll = $('seo_ping_all');
    if(!pingAll) return;
    $('seo_ping_all').onclick    = ()=> ping('all');
    $('seo_ping_google').onclick = ()=> ping('google');
    $('seo_ping_bing').onclick   = ()=> ping('bing');
    $('seo_indexnow_send').onclick = sendIndexNow;
    $('seo_websub_publish').onclick = publishWebSub;
    $('seo_add_feed').onclick = addToFeed;
  }
  async function ping(engine){
    const sitemap = $('seo_sitemap').value.trim();
    if(!sitemap) return alert('Indica o URL do sitemap');
    const res = await jfetch(`/api/seo/ping?sitemap=${encodeURIComponent(sitemap)}&engine=${engine}`);
    $('seo_ping_out').textContent = JSON.stringify(res, null, 2);
  }
  async function sendIndexNow(){
    const urls = $('seo_indexnow_urls').value.split(/\s+/).filter(Boolean);
    if(!urls.length) return alert('Indica pelo menos 1 URL');
    const res = await jfetch('/api/seo/ping', { method:'POST', body: JSON.stringify({ urls }) });
    $('seo_indexnow_out').textContent = JSON.stringify(res, null, 2);
  }
  async function publishWebSub(){
    const topic = $('seo_topic').value.trim();
    if(!topic) return alert('Indica o feed (topic)');
    const res = await jfetch('/api/seo/websub/publish', { method:'POST', body: JSON.stringify({ topic }) });
    $('seo_websub_out').textContent = JSON.stringify(res, null, 2);
  }
  async function addToFeed(){
    const key = $('seo_admin_key').value.trim();
    const url = $('seo_upd_url').value.trim();
    const title = $('seo_upd_title').value.trim();
    const summary = $('seo_upd_summary').value.trim();
    if(!key) return alert('Cola a ADMIN_API_KEY');
    if(!url) return alert('Indica o URL atualizado');
    const r = await fetch('/api/admin/add-update', {
      method:'POST',
      headers: { 'Content-Type':'application/json', 'x-admin-key': key },
      body: JSON.stringify({ url, title: title || undefined, summary: summary || undefined })
    });
    const txt = await r.text();
    $('seo_addfeed_out').textContent = txt;
  }
  document.addEventListener('DOMContentLoaded', mount);
})();
// === end SEO Ping & WebSub (injected) ===
