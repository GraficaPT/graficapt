...
/**
 * Build static product pages with server-rendered SEO.
...
async function main() {
  ...
  const { data: products, error } = await supabase
    .from('products')
    .select('slug, name, nome, description, descricao, metawords, image_url, cover, thumb, updated_at')
    .eq('active', true);
  ...
  for (const p of products || []) {
    ...
    const img = p.image_url || p.cover || p.thumb || `${BASE_URL}/imagens/social/logo_minimal.png`;
...
<meta property="og:image" content="${img.startsWith('http') ? img : escapeHtml(img)}">
...
`;
    ...
    // Inject a small bootstrap with product data (optional for client JS)
    const bootstrap = `<script>window.__PRODUCT__=${JSON.stringify({
      slug, name, desc, url, img, keywords
    })};</script>`;

    // Na versão básica, injetamos o headTags antes do fechamento de </head>
    let html = tpl.replace(/<\/head>/i, `${headTags}\n</head>`);

    // E adicionamos o bootstrap antes do fechamento do body
    html = html.replace(/<\/body>/i, `${bootstrap}\n</body>`);

    // Output to /produto/<slug>/index.html
